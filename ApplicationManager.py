...
class ApplicationManager:
	...
	def initVirtualApplication(self, mon,virtualMachines):

		for vm in virtualMachines:
			app = self.getAppInfo(vm)
			app.setIPC(mon.getIPC(app.cores))
			app.setMPI(mon.getMPI(app.cores))
			self.setConfigValues(app)
			self.virtualApplications.append(app)
		return self.virtualApplications

	def save(self,mon, apps):
		fileNameUtil = "expt/"+str(self.ts) + "cpu_Util" + ".csv"

		stat, percent=mon.getCPUStat(perCPU=True)
		#CPU stat save
