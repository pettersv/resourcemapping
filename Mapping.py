...
class Mapping:
    ...
    def calculate_mapping(self,time):
        ...
        virtualMachines = self.mon.getCurrentState()
        randomMapping = RandomMapping()

        map = MappingAlgorithm()

        vcpu_map,mem_map = map.get_vpcu_map(self.mon, virtualMachines, "IPC", 0.01,time) #MPI/ IPC
        return vcpu_map,mem_map

    #def calculate_mem_map(self):
    ...
    def start_mem_pin(self,time):
        ...
        # vcpu_map = randomMapping.get_vpcu_map(virtualMachines)
        map = MappingAlgorithm()
        vcpu_map, mem_map = map.get_mem_map(self.mon,virtualMachines,time)
    ...
    def startVanila(self,time):
        ...
        self.mon.probeWPerformance()
        virtualMachines = self.mon.getCurrentState()
        map = MappingAlgorithm()

        map.vanila(self.mon, virtualMachines,time)




    def start(self,algorithm,time):
